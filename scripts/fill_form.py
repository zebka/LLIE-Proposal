...
SRC = PROJ / "src"
FORM = BASE / "forms" / "PhDProposalform.docx"
OUT = BASE / "forms" / "PhDProposalform-Completed.docx"
...
RISK_ROWS = [
    ["ریسک", "راهکار"],
    ["تأخیر در جمع‌آوری دیتاست TwilightDrive", "روش و مقاله اول صرفاً با بنچمارک‌های موجود منتشر می‌شود؛ دیتاست در فاز ۳ و ۴ (بخش ۶) قرار دارد"],
    ["محدودیت حافظه GPU برای استنتاج diffusion", "استنتاج روی فضای نهفته/موجک [jiang2023diffll] و اجرای سنگین‌ها روی Colab L4"],
...
TIMELINE_ROWS = [
    ...
    ["۰", "۱ تا ۳", "تکمیل مرور ادبیات؛ پیاده‌سازی و بازتولید baselineها (Retinexformer، SCI، Cho et al.)؛ ساخت مجموعه ارزیابی اولیه گرگ‌ومیش برای تحلیل خرابی", "پروپوزال نهایی، کد baseline، گزارش تحلیل خرابی (پاسخ س۱)"],
    ["۱", "۴ تا ۸", "طراحی و پیاده‌سازی ماژول گیت روشنایی و prior فرکانسی؛ ارزیابی اولیه روی بنچمارک‌های موجود", "مدل ساده، مقاله کنفرانسی"],
    ["۲", "۹ تا ۱۴", "توسعه کامل سه ماژول؛ ablation و بهینه‌سازی استنتاج تطبیقی؛ ارزیابی جامع روی بنچمارک‌های استاندارد", "نسخه میانی روش، پیش‌نویس مقاله مجله اول"],
    ["۳", "۱۵ تا ۱۹", "ضبط و کیوریشن TwilightDrive (پروتکل بخش ۴-۱)؛ ارزیابی پایین‌دستی (تشخیص شیء)", "مقاله مجله اول، نسخه اولیه دیتاست"],
    ["۴", "۲۰ تا ۲۴", "تکمیل، ناشناس‌سازی و در صورت احراز شرایط، انتشار دیتاست و کد؛ مقاله دوم؛ نگارش رساله", "مقاله دوم، دیتاست منتشرشده و رساله تکمیل‌شده"],
]

MILESTONE = ("نقاط کنترل: پایان فاز ۱ = تصمیم go/no-go بر اساس نتایج فرضیه ۱؛ "
             "پایان فاز ۳ = ارزیابی فرضیه‌های ۲ و ۳؛ پایان فاز ۴ = دفاع بر اساس دو مقاله و دیتاست منتشرشده.")

TABLES = {
    "1": [
        ["دوره", "رویکرد", "نمایندگان", "محدودیت اصلی"],
        ["۱۹۷۱ تا ۱۹۷۷", "تئوری Retinex", "Land و McCann [land1977retinex]", "چارچوب نظری؛ حل تحلیلی به‌دلیل عدم یکتایی تجزیه ممکن نیست"],
        ["دهه ۱۹۸۰", "تراز هیستوگرام تطبیقی", "AHE/CLAHE [pizer1987clahe]", "تقویت نویز و اغتشاش رنگ در نواحی ناهموار"],
        ["۱۹۹۷", "Retinex چندمقیاس", "SSR/MSRCR [jobson1997msrcr]", "حساسیت به پارامترهای گوسی و رنگ مصنوعی"],
        ["۲۰۱۱", "قیاس با برداشتن غبار", "Dong et al. [dong2011dehaze]", "فرض مدل پراکندگی در صحنه کم‌نور واقعی برقرار نیست"],
        ["۲۰۱۶", "مدل واریانس‌پذیر وزن‌دار", "SRIE [fu2016srie]", "حل‌کننده تکراری کند؛ تنظیم دستی پارامترها"],
        ["۲۰۱۷", "تخمین نگاشت روشنایی", "LIME [guo2017lime]", "وابستگی به prior ساختاری دست‌ساز"],
        ["۲۰۱۷", "نخستین یادگیری عمیق", "LLNet [lore2017llnet]", "ظرفیت محدود autoencoder و آموزش ناپایدار"],
    ],
    "2": [
...
def main():
    if not FORM.exists():
        sys.exit("فرم مبدأ پیدا نشد: " + str(FORM) +
                 "\nنسخهٔ بکاپ را با همین نام در پوشهٔ forms بگذارید، "
                 "یا از scripts/build_docs.py استفاده کنید (خروجی مستقل + LaTeX).")
    ...
    # comparison table is now part of sec5 via the [[TABLECOMP]] marker in the md
