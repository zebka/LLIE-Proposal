...
EQ: z_T^*=σ(z_T^s)·(z_T^c-μ(z_T^c))/σ(z_T^c)+μ(z_T^s)#(۱)
...
EQ: L̂=blur_(5×5)(max_c⁡I_c)#(۲)

دو ماسک گیت سیگموئیدی، نواحی کم‌نورده و پرنورده را وزن‌دهی می‌کنند:

EQ: G_dark=σ((τ_low-L̂)/s), G_bright=σ((L̂-τ_high)/s)#(۳)
...
EQ: ε̃_t=ε_θ(z_t,t)+λ_d(t)·G_dark⊙(μ_E-μ_local(ẑ_(0,t)))-λ_b(t)·G_bright⊙max(μ_local(ẑ_(0,t))-μ_E,0)#(۴)
...
EQ: I_out=VAE.Dec(IDWT(ẑ_0,H_L))#(۵)
